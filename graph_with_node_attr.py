...
def preprocess_graphs(G, graphs_num):
    ...
    for i in range(graphs_num):
        ...
        for idx, edge in enumerate(ig_G.es):
            ...
            weight = norm(node_attr[u] - node_attr[v])
            
            # distance = norm(node_attr[u] - node_attr[v]) ** 2
            # weight = np.exp(- distance / 2)
            
            # cost = ot.dist(np.array([node_attr[u]]), np.array([node_attr[v]]), 'euclidean')
            # wd = ot.emd2([], [], cost)
            # weight = np.exp(-0.001*wd)
...
if __name__ == '__main__':
    ...
    # time_end = time.time()
    # print('time(k=%d, d=%d) is %.2fs' % (maxh - 1, depth - 1, (time_end - time_start)))
